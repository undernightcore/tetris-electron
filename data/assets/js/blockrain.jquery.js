...
(function ($) {
	...
	$.widget('aerolab.blockrain', {
		...
		updateSizes: function () {
			...
			this._BLOCK_HEIGHT = Math.floor(
				(this.element.innerHeight() / this.element.innerWidth()) *
					this._BLOCK_WIDTH
			);
		...
		theme: function (newTheme) {
			...
			if (
				isNaN(parseInt(this._theme.strokeWidth)) ||
				typeof parseInt(this._theme.strokeWidth) !== 'number'
			) {
		...
		_drawBackground: function () {
			...
			if (this._theme.backgroundGrid instanceof Image) {
				// Not loaded
				if (
					this._theme.backgroundGrid.width === 0 ||
					this._theme.backgroundGrid.height === 0
				) {
					return;
				}
				...
				for (var x = 0; x < this._BLOCK_WIDTH; x++) {
					for (var y = 0; y < this._BLOCK_HEIGHT; y++) {
						...
						this._ctx.drawImage(
							this._theme.backgroundGrid,
							0,
							0,
							this._theme.backgroundGrid.width,
							this._theme.backgroundGrid.height,
							cx,
							cy,
							this._block_size,
							this._block_size
				...
				for (var x = 0; x < this._BLOCK_WIDTH; x++) {
					for (var y = 0; y < this._BLOCK_HEIGHT; y++) {
						...
						this._ctx.fillRect(
							cx + borderWidth,
							cy + borderWidth,
							this._block_size - borderWidth * 2,
							this._block_size - borderWidth * 2
						);
					}
				}
				...
				$.extend(this, {
					...
					getBlocks: function (orientation) {
						...
						return this.orientations[
							orientation !== undefined ? orientation : this.orientation
						];
					},
				...
				checkForClears: function () {
					...
					for (i = 0, len = this.data.length; i < len; i++) {
						...
						if (
							...
							typeof this.data[i].blockType === 'string'
						) {
							count += 1;
						}
				...
				nextShape: function (_set_next_only) {
					...
					if (typeof game._theme.complexBlocks !== 'undefined') {
						if ($.isArray(game._theme.complexBlocks[result.blockType])) {
							result.blockVariation = game._randInt(
								0,
								game._theme.complexBlocks[result.blockType].length - 1
							);
						} else {
						...
						if ($.isArray(game._theme.blocks[result.blockType])) {
							result.blockVariation = game._randInt(
								0,
								game._theme.blocks[result.blockType].length - 1
							);
						} else {
							result.blockVariation = null;
						}
				...
				createRandomBoard: function () {
					...
					for (i = 0, ilen = game._BLOCK_WIDTH; i < ilen; i++) {
						for (
							...
							if (!blockType || !game._randInt(0, 3))
								blockType = game._randChoice(blockTypes);

							// Use a random piece and orientation
							// Todo: Use an actual random variation
							game._filled.add(
								i,
								game._BLOCK_HEIGHT - j,
								blockType,
								game._randInt(0, 3),
								null,
								game._randInt(0, 3)
          ...
          */
				...
				drawBlock: function (
					...
					if (color instanceof Image) {
						...
						if (
							typeof game._theme.blocks !== 'undefined' &&
							game._theme.blocks !== null
						) {
							game._ctx.drawImage(
								color,
								0,
						...
						// A custom texture
							...
							typeof game._theme.complexBlocks !== 'undefined' &&
							game._theme.complexBlocks !== null
						) {
							if (typeof blockIndex === 'undefined' || blockIndex === null) {
								blockIndex = 0;
							}

							var getCustomBlockImageCoordinates = function (
								image,
								blockType,
								blockIndex
							) {
							...
							var coords = getCustomBlockImageCoordinates(
								color,
								blockType,
								blockIndex
							);
						...
						if (typeof game._theme.innerStroke === 'string') {
							...
							game._ctx.fillRect(
								x + borderDistance,
								y + borderDistance,
								game._block_size - borderDistance * 2,
								borderWidth
							);
							// The rects shouldn't overlap, to prevent issues with transparency
							game._ctx.fillRect(
								x + borderDistance,
								y + borderDistance + borderWidth,
								borderWidth,
								game._block_size - borderDistance * 2 - borderWidth
							);
						}
						if (typeof game._theme.innerSquare === 'string') {
							...
							game._ctx.fillRect(
								x + squareDistance,
								y + squareDistance,
								game._block_size - squareDistance * 2,
								game._block_size - squareDistance * 2
							);
						}
					}
				...
				getBlockColor: function (
					...
					var getBlockVariation = function (blockTheme, blockVariation) {
						if ($.isArray(blockTheme)) {
							if (
								blockVariation !== null &&
								typeof blockTheme[blockVariation] !== 'undefined'
							) {
					...
					if (falling) {
						if (
							typeof game._theme.primary === 'string' &&
							game._theme.primary !== ''
						) {
							...
							return getBlockVariation(
								game._theme.blocks[blockType],
								blockVariation
							);
						} else {
							return getBlockVariation(
								game._theme.complexBlocks[blockType],
								blockVariation
							);
						}
					} else {
						if (
							typeof game._theme.secondary === 'string' &&
							game._theme.secondary !== ''
						) {
		...
		_preloadThemeAssets: function () {
			...
			if (typeof this._theme.complexBlocks !== 'undefined') {
				...
				for (var i = 0; i < keys.length; i++) {
					this._theme.complexBlocks[keys[i]] = startAssetLoad(
				...
				// Load the blocks
				for (var i = 0; i < keys.length; i++) {
					this._theme.blocks[keys[i]] = startAssetLoad(
		...
		_createHolder: function () {
			...
			this._$canvas = $(
				'<canvas style="display:block; width:100%; height:100%; padding:0; margin:0; border:none;" />'
			);
			if (typeof this._theme.background === 'string') {
				this._$canvas.css('background-color', this._theme.background);
			}
		...
		_createControls: function () {
			...
			game._$touchLeft = $(
				'<a class="blockrain-touch blockrain-touch-left" />'
			).appendTo(game._$gameholder);
			game._$touchRight = $(
				'<a class="blockrain-touch blockrain-touch-right" />'
			).appendTo(game._$gameholder);
			game._$touchRotateRight = $(
				'<a class="blockrain-touch blockrain-touch-rotate-right" />'
			).appendTo(game._$gameholder);
			game._$touchRotateLeft = $(
				'<a class="blockrain-touch blockrain-touch-rotate-left" />'
			).appendTo(game._$gameholder);
			game._$touchDrop = $(
				'<a class="blockrain-touch blockrain-touch-drop" />'
			).appendTo(game._$gameholder);
		},

		_refreshBlockSizes: function () {
			if (this.options.autoBlockWidth) {
				this.options.blockWidth = Math.ceil(
					this.element.width() / this.options.autoBlockSize
				);
			}
		},
		...
		_getNiceShapes: function () {
			...
			function scoreBlocks(possibles, blocks, x, y, filled, width, height) {
				...
				for (i = 0; i < len; i += 2) {
					score +=
						possibles[game._filled.asIndex(x + blocks[i], y + blocks[i + 1])] ||
						0;
				}
		...
		_setupControls: function (enable) {
			...
			function isStopKey(evt) {
				...
				var isStop =
					cfg.stopKeys[evt.keyCode] ||
					(cfg.moreStopKeys && cfg.moreStopKeys[evt.keyCode]);
		...
		_setupTouchControls: function (enable) {
			...
			if (!game.options.autoplay && enable) {
				game._$touchLeft
					...
					.bind('touchend', endMoveLeft);
				game._$touchRight
					.show()
					.bind('touchstart click', moveRight)
					.bind('touchend', endMoveRight);
				game._$touchDrop
					.show()
					.bind('touchstart click', drop)
					.bind('touchend', endDrop);
